...
### Q1: 这个网站支持中文吗？
- **UI 界面**：当前前端界面菜单及提示默认为英文，暂未提供多语言（i18n）切换开关。
- **中文预览**：**完全支持！** 可以在顶部搜索/预览栏中直接输入中文文本（如“字体排版之美”），页面会即时呈现对应的中文字形。
- **中文字体解析**：后端解析器能正确识别中文字体的名称（Family Name / Full Name），中文字体名支持关键词模糊检索。
...
│   ├── src/
│   │   ├── components/     # UI 组件 (FontCard, Sidebar 等通用组件)
