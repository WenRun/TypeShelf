...
- 🏷️ **字体标签与智能分类系统**：
  - **自动规则识别**：内置 13 种常见字体风格与特性分类（黑体、宋体、楷体、圆体、书法手写、卡通可爱、艺术海报、等宽、可变、斜体等），导入时自动精准打标。
  - **自定义手动打标**：字体详情页支持一键添加推荐预设标签或创建任意自定义标签，支持一键移除。
  - **全链路导航与筛选**：侧边栏标签分类导航实时统计数量，首页横向胶囊栏一键筛选，支持 `#标签` 前缀复合搜索。
  - **AI 智能分析大模型集成**：支持在「系统设置」配置兼容 OpenAI 格式的大语言模型服务商（预设 DeepSeek、OpenAI、通义千问、月之暗面、本地 Ollama 等），支持连通性测试与延时探测，在字体详情页一键调用大模型深度解析设计风格与场景应用并生成专业标签，具备全链路双轨降级兜底。
...
- **统一集中存放（推荐）**：
  - **本地运行**：将你的字体文件（`.ttf`, `.otf`, `.woff`, `.woff2`, `.ttc`）放入项目根目录下的 `fonts/` 文件夹内（支持多层子目录嵌套）。
  - **Docker 部署**：将宿主机的字体文件夹直接挂载至容器内的 `/app/fonts`（例如 `-v /vol1/1000/fonts/fonts:/app/fonts`）。
- **全自动识别与热监听**：
  - 放入字体后，服务端内置的实时文件监听引擎（Chokidar）会自动检测新增并完成解析；
  - 亦可在 Web 界面右上角点击 **刷新扫描图标（↻）** 手动触发全库增量重扫；
  - 界面已移除冗余复杂的“字体目录/路径选择”配置，字体的组织与归类统一通过更现代化、灵活的**【合集分组】**、**【我的收藏】**以及**【智能分类与多维标签体系】**完成。
