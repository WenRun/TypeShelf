...
- [二、核心技术模块设计与实现机制](#二核心技术模块设计与实现机制)
  ...
  - [2.9 全端移动端响应式适配设计架构 (Mobile Responsive Architecture)](#29-全端移动端响应式适配设计架构-mobile-responsive-architecture)
...
- [四、常见问题排查手册与故障排解指南 (Troubleshooting & FAQ)](#四常见问题排查手册与故障排解指南-troubleshooting--faq)
  ...
  - [Q20: 移动端（手机/平板）访问时页面挤压变形与触控交互不可用的原因及全站自适应改造方案](#q20-移动端手机平板访问时页面挤压变形与触控交互不可用的原因及全站自适应改造方案)
...
### 2.9 全端移动端响应式适配设计架构 (Mobile Responsive Architecture)
- **设计目标**：
  让 RunFonts 在智能手机（360px ~ 480px）、平板（768px ~ 1024px）与高分辨率桌面显示器（1280px+）上均能提供原生 App 般舒适的排版与交互体验，解决移动端边栏挤压主视图、横向滚动溢出、固定抽屉无法遮罩、触控设备缺乏 hover 状态导致操作按钮不可见等缺陷。
- **架构与模块实现策略**：
  1. **响应式边栏与抽屉架构 (Sidebar & Mobile Drawer)**：
     - 将原单一的 PC 端静态 `<Sidebar>` 拆解为可复用的核心渲染器 `<SidebarContent>` 与适配容器；
     - PC 端保持 `<aside className="hidden md:flex w-64 h-screen ...">`；
     - 移动端基于 Radix UI `Sheet` 组件构建 `<MobileSidebar>`，点击汉堡菜单以原生抽屉方式从左侧滑出，支持滑动手势关闭及路由跳转自动关闭（`onNavigate`），并适配移动端安全视口；
  2. **首页视口与工具栏动态折叠 (Home Fluid Layout)**：
     - 头部 Header 采用双层响应式栅格：小屏时第一行展示汉堡导航按钮、应用标题与快速操作项（实时预览展开开关、重新扫描、主题/语言切换），第二行全宽展示搜索输入框；
     - 胶囊标签栏采用触摸滚动条优化（`-webkit-overflow-scrolling: touch; scrollbar-width: none;`），支持在小屏横向平滑划动；
     - 字体瀑布流卡片采用自适应列宽网格（`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`），并在小屏自适应收紧内边距与预览字号；
  3. **字体详情页双轨检查器 (FontDetail Dual-Track Inspector)**：
     - PC 宽屏端保持右侧常驻面板 `<aside className="hidden lg:flex w-80 ...">`；
     - 移动端将详细参数、文件列表与代码调用抽离进右侧抽屉 `<Sheet>`，顶部提供带提示圆点的高清操作按钮，小屏用户点击随时滑出，主视口空间 100% 留给字体排版与缩放试炼；
     - 交互预览控制区改为自适应折叠布局（单列纵向堆叠试字框与字号滑块）；
  4. **触控交互体验优化 (Touch Hover Decoupling)**：
     - 解决手机/平板触控屏无法触发 `:hover` 伪类的关键缺陷：对卡片收藏按钮、合集菜单按钮、标签删除角标等采用 `opacity-100 sm:opacity-0 sm:group-hover:opacity-100` 样式策略，在触控端全时常驻显示或即点即用，在桌面端保持纯净极简的悬浮展开动效；
  5. **设置中心全端适配 (Responsive Settings)**：
     - 小屏增加移动端轻量导航条与汉堡入口；
     - 表单操作按钮区在小屏下由横向浮动转为纵向铺满（`flex-col-reverse sm:flex-row w-full sm:w-auto`），方便单手拇指直观点击。
...
### Q20: 移动端（手机/平板）访问时页面挤压变形与触控交互不可用的原因及全站自适应改造方案
- **现象描述**：
  1. 手机竖屏（如 iPhone、Android 手机）访问网站时，左侧 256px 宽度的边栏直接占据了超过 60% 的屏幕宽度，导致右侧字体列表被严重压缩成极窄条状甚至溢出横向滚动条；
  2. 字体详情页同时存在左侧边栏与右侧 320px 参数检查器，在移动端访问时中间字体预览区域彻底无法显示；
  3. 触控屏上没有鼠标指针 hover 悬浮事件，导致字体卡片的“收藏”、“加入合集”以及已打标签的“删除 (X)”按钮因 `opacity-0 group-hover:opacity-100` 样式无法显示与点击；
  4. 移动端软键盘弹起时，传统视口单位导致页面发生意外拉伸变形与滚动跳跃。
- **根因分析**：
  - 原开源项目未针对 `< 768px` 的视口媒体查询与触控特性做响应式设计（Mobile-First / Responsive Web Design）；
  - 核心布局硬编码了固定的 `w-64`、`w-80` 以及绝对定位与悬浮触发机制。
- **全链路改造与落地**：
  1. **架构拆分与抽屉化**：引入 `@/components/ui/sheet`，侧边栏与详情页检查器在 `< md` 与 `< lg` 视口下转为模态抽屉，桌面端恢复无损静态多列布局；
  2. **视口与内边距流体化**：统一采用 Tailwind 响应式前缀（如 `p-4 sm:p-8`、`grid-cols-1 sm:grid-cols-2`、`max-w-[90vw]` 等）；
  3. **触控状态脱耦**：对所有关键操作按钮采用 `opacity-100 sm:opacity-0 sm:group-hover:opacity-100`，彻底解决触控设备由于没有 hover 导致功能按钮不可见不可用的缺陷；
  4. **顶部导航整合**：移动端首页与设置页提供自适应汉堡导航栏，统一多语言、深浅色模式与菜单呼出体验。
