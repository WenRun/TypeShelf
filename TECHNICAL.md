...
- **分阶段实施架构**：
  ...
  3. **Phase 3（AI 视觉多模态风格分析与大模型集成落地）**：
     ...
     - **全链路高可用降级矩阵**：若 AI 未启用、未配置 Key、网络断开、超时（20s）或响应格式异常，系统百分之百自动降级为规则引擎分析（`storage.autoTagFonts`)，返回 `status: "fallback_rule"` 并清晰告知用户降级原因，保障生产不中断。
      - **双层分类模型与 Prompt 契约**：
        - `tags`：3~4 个核心正式分类，必须包含 1 个基础字形门类（黑体、宋体、楷体、仿宋、圆体、书法手写、美术创意、等宽字体）与 2~3 个主视觉风格/应用场景；系统自动为字体创建并绑定这些正式分类。
...
2. **默认提示词**：系统开箱预置了专为字体排版艺术定制的 System Prompt，实行双层分类契约：`tags` 自动入库（1 项基础字形门类 + 2~3 项核心风格/场景），`suggestions` 输出 5~8 个动态候选并替换详情页静态推荐标签，供用户点击确认加入。
