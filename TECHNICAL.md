...
- **分阶段实施架构**：
  ...
  3. **Phase 3（AI 视觉多模态风格分析与大模型集成落地）**：
     ...
     - **大模型排版风格分析引擎 (`POST /api/fonts/:family/ai-tag`)**：
       ...
       - 结构化多维度 Prompt 引导：引导大模型输出分层标签矩阵——必须包含 1 个基础字形大类（黑体、宋体、楷体、仿宋、圆体、书法手写、美术创意、等宽字体）+ 2~3 个视觉风格与场景定位标签（如“现代极简”、“复古国风”、“商务办公”、“二次元可爱”、“电商海报”等）并附带简短分析理由；
       - 鲁棒 JSON 提取器：内置 Markdown 代码块与正则表达式容错解析，确保各种大模型格式均能稳定解析；
       - 自动入库与标记：解析成功后自动将新标签创建/匹配至系统标签库，并为字体打标（`source: "ai"`）；
...
2. **默认提示词**：系统开箱预置了专为字体排版艺术定制的 System Prompt，实行双维度强制约束（1 项基础字形类别 + 2~3 项风格场景定位），既保证包含黑体/宋体/楷体/手写等基础类型，又兼顾商业应用与美学场景；
