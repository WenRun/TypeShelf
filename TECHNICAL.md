...
- **分阶段实施架构**：
  ...
  3. **Phase 3（AI 视觉多模态风格分析与大模型集成落地）**：
     ...
     - **大模型排版风格分析引擎 (`POST /api/fonts/:family/ai-tag`)**：
       ...
       - 设计学 Prompt 引导：引导大模型从设计排版、平面视觉与场景应用角度，提炼 2~4 个精练专业的中文标签（如“复古国风”、“电商海报”、“商务黑体”、“二次元可爱”等）并附带简短分析理由；
...
2. **默认提示词**：系统开箱预置了专为字体排版艺术定制的 System Prompt，包含 11 种风格特征大类、7 种场景定位以及严格的合法 JSON 提取规范；
